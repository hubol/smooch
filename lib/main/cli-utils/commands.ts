...
${
        commandMetadatas.map(({ name, args, description }) =>
            `${chalk.yellow(name)}${args ? ` ${args}` : ""}
${description}`
        ).join("\n\n")
    }`);
};
...
export async function getCommandMetadatas() {
    return await Promise.all(
        Object.entries(cliUtilsCommands)
...
function cmd(
    ...
    argsOrDescription: string | Promise<string>,
...
const cliUtilsCommands: Record<string, Command> = {
    "init": cmd(initializeSmoochConfig, `Initialize a **smooch.json** configuration file.`),

    "copy-program": cmd(
        (src, dst) => copyTemplateProgram(src, Path.File.create(dst)),
        "<program> <dst>",
        (async () => {
...
${defaultsList}`;
    ...
    "init-native-deps": cmd(
        initializeNativeDepsConfig,
